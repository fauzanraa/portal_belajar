    ...
    public function storeQuestion($id, Request $request){
        ...
        try {
            ...
            for ($i=0; $i < $request->total_question; $i++) { 
                ...
                $systemPromptCase = "Anda adalah asisten yang sangat terstruktur dalam menggambar flowchart. Jawaban Anda hanya berupa JSON dalam format GoJS yang valid. Gunakan hanya dua array: nodeDataArray dan linkDataArray. Jangan sertakan penjelasan atau komentar apapun. Setiap flowchart harus dimulai dengan node Terminator (Start) dan diakhiri dengan Terminator (End). Gunakan Decision untuk kondisi dengan label Yes/No, dan gunakan komponen sesuai fungsi masing-masing.";
                $promptCase = "Buat expected result dari studi kasus berikut dalam bentuk JSON GoJS dengan dua bagian: nodeDataArray dan linkDataArray. Flowchart harus mengikuti aturan dengan komponen wajib: {$requiredComponents}. Soal: {$question->question}. Jangan sertakan apapun selain JSON.";
