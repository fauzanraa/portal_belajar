...
@section('content')
        ...
        <div class="w-full mb-6">
            <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
                <div class="flex items-center mb-4">
                    ...
                    <h2 class="text-2xl font-bold text-gray-800">Soal Flowchart</h2>
                </div>
                <div class="bg-blue-50 rounded-lg p-4">
                  @foreach ($sessionTask as $task)
                    @foreach ($task->taskQuestion as $data)
                      <p class="text-gray-700 leading-relaxed">{{$data->question}}</p>
                    @endforeach
                  @endforeach
                </div>
            </div>
    ...
    <script>
        ...
        // Help function
        function showHelp() {
            alert('Panduan:\n1. Drag komponen dari panel kiri ke area gambar\n2. Hubungkan komponen dengan menarik garis\n3. Klik dua kali pada komponen untuk mengedit teks\n4. Pilih selesai untuk menyimpan jawaban');
        }
...
@section('script')
    ...
    <script id="code">
        ...
        function saveFlowchartToDatabase() {
            ...
            const sessionTasks = @json($sessionTask);
            const idTeacher = @json($encryptedTeacher);
            const idMeeting = @json($encryptedMeeting);

            let soalData = [];

            sessionTasks.forEach(task => {
              task.task_question.forEach(data => {
                soalData.push({
                  question_id: data.id,  
                });
              });
            ...
            .then(data => {
                if (data.success) {
                    ...
                    setTimeout(() => {
                        const redirectUrl = '{{ route("detail-meetings", ["idTeacher" => ":teacher", "idMeeting" => ":meeting"]) }}'
                        .replace(':teacher', idTeacher)
                        .replace(':meeting', idMeeting);
