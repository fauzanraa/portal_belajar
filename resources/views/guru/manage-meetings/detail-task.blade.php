...
@section('content')
    ...
    <div class="mt-10 bg-white p-5 pl-8 rounded-l-xl">
        <div class="button mt-5 px-5 flex justify-self-end">
            <button data-modal-target="edit-task-modal" data-modal-toggle="edit-task-modal"
                class="px-5 py-2.5 text-sm rounded-lg bg-sky-500 text-sm hover:bg-sky-700 text-white cursor-pointer"
                onclick="openEditModal({{ $dataTugas->id }}, '{{ $dataTugas->name }}', '{{ $dataTugas->open_at }}', '{{ $dataTugas->close_at }}', '{{ $dataTugas->duration }}')">
            ...
            <div class="relative overflow-x-auto">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <tbody>
                        <tr class="bg-white">
                            <th scope="row"
                                class="px-6 py-4 w-1/4 font-medium border border-gray-400 bg-slate-300 text-black">
                                Judul
                            </th>
                        ...
                        <tr class="bg-white">
                            ...
                            <td class="px-6 py-4 border-y border-gray-200 border-r border-gray-200">
                                {{ $dataTugas->open_at ? \Carbon\Carbon::parse($dataTugas->open_at)->locale('id')->isoFormat('D MMMM YYYY') : '-' }}
                                ({{ $dataTugas->open_at ? \Carbon\Carbon::parse($dataTugas->open_at)->locale('id')->isoFormat('HH:mm') : '-' }})
                                -
                                {{ $dataTugas->close_at ? \Carbon\Carbon::parse($dataTugas->close_at)->locale('id')->isoFormat('D MMMM YYYY') : '-' }}
                                ({{ $dataTugas->close_at ? \Carbon\Carbon::parse($dataTugas->close_at)->locale('id')->isoFormat('HH:mm') : '-' }})
                        ...
                        <tr class="bg-white">
                            <th scope="row" colspan="2"
                                class="px-6 py-4 w-1/4 font-medium border border-gray-400 bg-slate-300 text-black text-center">
                                Pengaturan soal
                            </th>
                        </tr>
                        <tr class="bg-white">
                            <th scope="row" colspan="2"
                                class="px-6 py-4 w-1/4 font-medium border border-gray-200 text-black text-center">
                                @php
                                    $encryptedTask = Illuminate\Support\Facades\Crypt::encrypt($dataTugas->id);
                                @endphp
                                <div class="button flex gap-4 justify-self-end">
                                    @if ($dataSoal->count() > 0)
                                        <button data-modal-target="component-settings-modal"
                                            data-modal-toggle="component-settings-modal"
                                    ...
                                    <button
                                        class="px-5 py-2.5 text-sm rounded-lg bg-sky-500 text-sm hover:bg-sky-700 text-white cursor-pointer"
                                        onclick="window.location.href='{{ route('question-tasks', $encryptedTask) }}'">
                                            ...
                                            @foreach ($dataSoal as $data)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @php
                                                            $maxLength = 100;
                                                            $isLong = strlen(strip_tags($data->question)) > $maxLength;
                                                            $shortText = Str::limit(
                                                        ...
                                                        <div x-data="{ expanded: false }">
                                                            <span x-show="!expanded">
                                                                {{ $shortText }}
                                                                @if ($isLong)
                                                                    <button class="text-blue-600 text-xs underline"
                                                                        @click="expanded = true">Lihat lebih banyak</button>
                                                                @endif
                                                            </span>
                                                            <span x-show="expanded" x-cloak>
                                                                {{ $data->question }}
                                                                <button class="text-blue-600 text-xs underline"
                                                                    @click="expanded = false">Lihat lebih sedikit</button>
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        @php
                                                            $encryptedQuestion = Illuminate\Support\Facades\Crypt::encrypt(
                                                                $data->id,
                                                            );
                                                        @endphp

                                                        <button
                                                            onclick="window.location.href='{{ route('expected-answer', $encryptedQuestion) }}'"
                                                            class="cursor-pointer">
                                                            <i
                                                                class="bi bi-eye p-2 text-sm rounded-lg bg-sky-500 hover:bg-sky-700 text-white cursor-pointer mr-4">
                                                                Lihat</i>
                                                        </button>
                                                    ...
                                                    <td>
                                                        ...
                                                        @endphp
                                                        @if (!empty($data->correct_answer))
                                                            <a href="{{ route('draw-correct-answer', ['id' => $encryptedQuestion]) }}"
                                                                onclick="event.preventDefault(); showConfirmation('{{ route('draw-correct-answer', ['id' => $encryptedQuestion]) }}')"
                            ...
                            <td class="px-6 py-4 border-y border-gray-200 border-r border-gray-200">
                                ...
                                <div class="block mt-5 mb-2 flex gap-4">
                                    @php
                                        $encryptedTask = Illuminate\Support\Facades\Crypt::encrypt($dataTugas->id);
                                    @endphp
                                    <a href="{{ route('session-tasks', ['id' => $encryptedTask, 'type' => 'non_system']) }}"
                            ...
                            <td class="px-6 py-4 border-y border-gray-200 border-r border-gray-200">
                                ...
                                <div class="block mt-5 mb-2 flex gap-4">
                                    @php
                                        $encryptedTask = Illuminate\Support\Facades\Crypt::encrypt($dataTugas->id);
                                    @endphp
                                    <a href="{{ route('session-tasks', ['id' => $encryptedTask, 'type' => 'system']) }}"
                                        class="text-sky-500 hover:text-sky-700">
                                        <div class="block">
                                            <i class="bi bi-people-fill"></i><span class="ml-3">Atur sesi</span>
                                        </div>
                ...
                <div class="button mt-8 flex justify-self-end">
                    ...
                    <button type="button"
                        class="px-5 py-2.5 text-sm rounded-lg bg-sky-500 text-sm hover:bg-sky-700 text-white cursor-pointer"
                        onclick="window.location.href='{{ route('manage-materials', ['id' => $encryptedMeeting]) }}'">
    ...
    <div id="edit-task-modal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                <div
                    ...
                    <button type="button"
                        ...
                        data-modal-hide="edit-task-modal">
                ...
                <div class="p-4 md:p-5">
                    <form class="space-y-4" action="{{ route('update-tasks', ['id' => $dataTugas->id]) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div id="date-range-picker" class="flex items-center gap-3">
                                <div class="flex-1 space-y-2">
                                    <div class="relative">
                                        ...
                                        <input id="datepicker-range-start" datepicker datepicker-orientation="top"
                                    ...
                                    <div class="flex">
                                        <input type="time" id="time-start" name="time_start"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 leading-none focus:ring-blue-500 focus:border-blue-500 block flex-1 w-full text-sm rounded-s-lg p-2.5"
                                            required>
                                        <span
                                            class="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-s-0 border-gray-300 rounded-e-lg dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                                            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true"
                                                xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                                viewBox="0 0 24 24">
                                ...
                                <div class="flex-shrink-0 px-2">
                                    <span
                                        class="text-gray-500 dark:text-gray-400 font-medium text-sm whitespace-nowrap">sampai</span>
                                </div>
                                ...
                                <div class="flex-1 space-y-2">
                                    <div class="relative">
                                        ...
                                        <input id="datepicker-range-end" datepicker datepicker-orientation="top"
                                            name="end" type="text"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5"
                        ...
                        <div>
                            ...
                            <input type="text" inputmode="numeric" name="duration" id="duration"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                required />
                            <p class="text-red-300 text-xs pt-1">nb : tulis dalam satuan menit</p>
                        </div>
                        <button type="submit"
                            class="w-full text-white mt-1 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Simpan</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div id="component-settings-modal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                <div
                    ...
                    <button type="button"
                        ...
                        data-modal-hide="component-settings-modal">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            @foreach ($allComponents as $key => $label)
                                <div class="flex items-center">
                                    <input type="checkbox" id="component_{{ $key }}" name="components[]"
                                        value="{{ $key }}"
    ...
    <div id="flowchart-modal" tabindex="-1" aria-hidden="true"
        class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-lg dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5">
                    <div id="flowchart-container"
                        class="w-full h-96 border border-gray-300 rounded-lg overflow-auto bg-gray-50 flex items-center justify-center">
                        @php
                            $flowchartImg = $data->flowchart_img ?? null;
                        @endphp

                        @if ($flowchartImg)
                            <img src="{{ asset('storage/assets/flowcharts/correctAnswer/' . $flowchartImg) }}"
                                alt="Flowchart Siswa" class="max-w-full max-h-full object-contain rounded-lg shadow-sm">
                        @else
