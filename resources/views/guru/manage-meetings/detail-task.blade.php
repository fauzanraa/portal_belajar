...
@section('content')
    ...
    <div class="mt-10 bg-white p-5 pl-8 rounded-l-xl">
        <div class="button mt-5 px-5 flex justify-self-end">
            <button data-modal-target="edit-task-modal" data-modal-toggle="edit-task-modal" class="px-5 py-2.5 text-sm rounded-lg bg-sky-500 text-sm hover:bg-sky-700 text-white cursor-pointer" onclick="openEditModal({{ $data_tugas->id }}, '{{ $data_tugas->name }}', '{{ $data_tugas->open_at }}', '{{ $data_tugas->close_at }}', '{{ $data_tugas->duration }}')">
    ...
    <div id="edit-task-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <div class="p-4 md:p-5">
                    <form class="space-y-4" action="{{route('update-tasks', ['id' => $data_tugas->id])}}" method="POST">
                        @csrf
                        @method('PUT')
...
@section('script')
    ...
    <script> 
        ...
        document.getElementById('component-settings-form').addEventListener('submit', function(e) {
            ...
            fetch(`{{ route("update-components", $data_tugas->id) }}`, {
                method: 'POST',
                headers: {
